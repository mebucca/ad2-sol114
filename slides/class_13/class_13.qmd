...
format:
  revealjs:
    ...
    title-slide-attributes:
      data-background-color: "#A67C52"
---

## Intervalos de Confianza para medias y proporciones muestrales {background-color="#FAF3E0" .inverse .center .middle .section-title}
...
[Intervalos de Confianza comunmente usados:]{.bold}
...
[Trade-off entre precisión y confianza]{.bold}

$$IC: \bar{X} \pm Z_{(\alpha/2)} \cdot \sigma/\sqrt{n}$$

- $\downarrow  \alpha\implies \uparrow Z_{\alpha/2}$: Un nivel de confianza más alto (mayor seguridad de que intervalo contiene el parámetro poblacional) conduce a un intervalo de confianza más amplio (menor precisión).
...
## Intervalos de confianza {background-color="#FAF3E0" .inverse .center .middle .section-title}
## Ejemplo
...
## Intervalos de Confiaza para proporciones {background-color="#FAF3E0" .inverse .center .middle}
...
- Usamos la proporción muestral $\hat{p} = \frac{\sum_{i=1}^{n}X_{i}}{n}$ como un estimador de la probabilidad de éxito en la población.
...
::: {.pull-left}
[Valor esperado]{.bold} de una Bernoulli:
...
::: {.pull-right}
[Varianza]{.bold} de una Bernoulli:
...
* [Varianza máxima]{.bold} cuando $p=0.5 \implies p(1-p)=0.25$
...
- La proporción muestral es: $\hat{p}_{n}=  \frac{\sum_{i=1}^{n}X_{i}}{n} \equiv \bar{X}$, donde las $X$'s son Bernoulli y [iid]{.bold}.
...
En resumen, $\hat{p}_{n}$ es un estimador insesgado y consistente de $p$.
...
## Hasta la próxima clase. Gracias! {background-color="#FAF3E0" .inverse .center .middle}
