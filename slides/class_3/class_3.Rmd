...
table(X) %>% as_tibble() %>% mutate(p=round(n/sum(n),2)) %>% select(-n) %>%
...
La distribución de una variable se puede caracterizar de tres modos:

- .bold[Función de densidad/masa de probabilidad] (pdf): $f(x)$

   - En el caso de variables discretas $f(x) = \mathbb{P}(X=x)$. Ejemplo, evaluando $f(5)$ obtenemos la probabilidad de que $X$ tome valor 5.

--

- .bold[Función densidad/masa de probabilidad acumulada] (cdf): $F(x) = \mathbb{P}(X \leq x)$

  - Ejemplo, evaluando $F(5)$ obtenemos la probabilidad de que $X$ tome un valor igual o menor 5.

--

- .bold[Función cuantil]: $Q(p) = F^{-1}(p)$

  - Ejemplo, evaluando $Q(0.1)$ obtenemos el valor de $x$ tal que la probabilidad de que $X$ tome un valor igual o menor a $x$ es 10\%.
