...
ggplot(data = pmf_df, aes(x = factor(x), y = pmf)) +
  geom_bar(stat = "identity", color = "black", fill = "#FBA72A", width = 1) +
  labs(x = "Outcome",
       y = "P(X = x)") +
...
plot <- plot + 
  stat_function(fun = binom_distrib, args = list(n= 2, p= 0.5), 
                aes(color="#FBA72A"), size=1.5) + # Notice the change in color to match the Set1 palette.
...
# Create the ggplot
ggplot(data = pdf_df, aes(x = x, y = pdf)) +
  geom_area(fill="#FBA72A") +
  geom_line(color = "black", size = 1) +
  geom_rect(aes(xmin = a, xmax = b, ymin = 0, ymax = 1 / (c - a)), color="black",        fill= "#E54E21", alpha=0.05) +
