...
# Distribución t-student {background-color="#ADD8E6" .inverse .center .middle}
...
- Dos problemas: 
  ...
  - Muestras pequeñas: la empresa no quería desperdiciar cerveza en experimentos.
...
- [Tamaño Muestral]{.bold}: Cuando se trabaja con muestras pequeñas ( $\approx n < 30$ ), los resultados estadísticos tienden a ser menos precisos.

<br>

- [Varianza Desconocida]{.bold}: Cuando no conocemos la **varianza poblacional** ( $\sigma^2$ ), debemos estimarla a partir de los datos: $s^2$. Esto introduce una fuente adicional de incertidumbre, ya que $s^2$ es una estimación basada en datos limitados.

<br>
- Relación entre [tamaño muestral]{.bold} y [varianza desconocida]{.bold}: cuando usamos la **varianza muestral** en lugar de la varianza poblacional, perdemos un **grado de libertad**. 
...
- Probabilidad asignada a valores extremos es mayor mientras más pequeña es la muestra.

- La distribución $t$ es, por tanto, más conservadora

---

### Distribución $t$ Estándar
...
[Paso a Paso del test de hipótesis]{.bold}
...
## Ejemplo: Calorías en bebidas de Starbucks {background-color="#ADD8E6" .inverse .center .middle}
...
library(tidyverse)
starbucks_menu <- read_csv("starbucks.csv")
...
- Como no conocemos $\sigma$ debemos estimarlo a partir de la muestra ($s$).
...
- Dado que $H_1: \mathbb{E}(X) > 100 \text{ kcal}$, tiene sentido testear si nuestro estadístico se encuentra en el $\alpha$% más alto de la distribución nula.
