...
- $\hat{\mu} =  \frac{\sum \text{educ}_{i}}{n} = 12.93$ es nuestro *estimado* 
...
- Tomamos 5000 muestras alesatorias de tamaño 200 a partir de esta población: $m_{200_i}$
...
for (i in 1:5000) {
  ...
  muestra_i <- sample_n(educ_poblacion, size=200, replace = TRUE)
...
- En nuestro caso:
  ...
  $$\bar{X}_{200} \sim \text{Normal}\bigg(12.85, \frac{3.464}{\sqrt{200}}\bigg)$$
...
y_values <- dnorm(x_values, mean = 12.85, sd = 3.464/sqrt(200))
...
$\bar{X}_{200}$ =  `r c(mu_hat = mean(mimuestra$educ, na.rm = F))`
...
normal_distribution_plot <- ggplot(normal_data, aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = 12.85, linetype = "dashed", size=1.5) +
  annotate("text", x = 13.1, y = 0.25, label = expression(mu), size = 12) +
  geom_point(aes(x=mean(mimuestra$educ, na.rm = F),y=dnorm(mean(mimuestra$educ, na.rm = F), mean = 12.85, sd = 3.464/sqrt(200))), size=5, colour="red") +
  labs(
       x = expression(bar(X)),
       y = "Densidad") +
