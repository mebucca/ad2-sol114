...
subtitle: "Test de Hipótesis en una muestra"
...
format:
  revealjs:
    ...
    title-slide-attributes:
      data-background-color: "#008C45"
---

# Test de Hipótesis en una muestra {background-color="#3949AB" .inverse .center .middle}
...
[Hipótesis Nula]{.bold}: "Paul no tiene ningún talento especial, sus predicciones son como tirar una moneda al aire".
...
[Hipótesis Alternativa]{.bold}: "Paul tiene un talento, acierta más de lo que falla", o quizás 
...
- Estandarizando podemos  derivar un [test-z]{.bold}:

  - Si $Z_{\hat{p}} = \frac{\hat{p} - p}{\frac{\sqrt{p(1-p)} }{\sqrt{n}}}$ entonces: $Z_{\hat{p}} \mid H_0   \sim \text{Normal}\big(0, 1\big)$

. . .

  - Nuestro estadístico $\hat{p}$ se transforma el [test-z]{.bold}, el valor estandarizado de del estadístico bajo $H_0$:
...
- Mientras menor es $\alpha$ más difícil es rechazar la hipótesis nula.
...
- Comparamos -- bajo el supuesto de que $H_0$ es verdadera --  la probabilidad de obtener el resultado que obtuvimos ( valor-p ) vs. el criterio seleccionado para clasificar resultados como "extremos" ( $\alpha$ ).
...
## {background-color="#3949AB" .inverse .center .middle}
...
## Hasta la próxima clase. Gracias! {background-color="#3949AB" .inverse .center .middle}
