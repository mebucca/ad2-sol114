...
## Distribución Normal Standard {background-color="#FFFFF0" .inverse .center .middle .section-title}

## Distribución Normal Standard

. . .

- $X$ es una variable aleatoria que sigue una distribución Normal con valor esperado $\mu$ y desviación estándar $\sigma$.
...
[Pregunta: ¿Cual es la distribución de Z?]{.bold}
...
::: {.pull-right}
[Función de densidad probabilística]{.bold}
...
::: {.pull-right}
[Función de probabilidad acumulada (CDF):]{.bold}
...
- Por convención la [CDF de una Normal Standard se denota]{.bold} $\Phi(.)$, tal que 
...
## Propiedades de la Normal Standard {background-color="#FFFFF0" .inverse .center .middle .section-title}
...
- La probabilidad de encontrar [valores a menos de 1σ]{.bold} respecto de la media es aprox. 68\%.
...
- La probabilidad de encontrar [valores a menos de 2σ]{.bold} respecto de la media es aprox. 96\%.
...
## Quantiles de la Normal Standard {background-color="#FFFFF0" .inverse .center .middle .section-title}
...
::: {.pull-right}
En terminos generales, la [función de quantiles]{.bold} es la inversa de la función de densidad acumulada (CDF): 
...
[Ejemplo:]{.bold} ¿Bajo que valor de una Normal Standard se acumula el 90% de la probabilidad?
...
## De Normal Standard a {background-color="#FFFFF0" .inverse .center .middle .section-title}
...
Si sabemos que $Z \sim \text{Normal}(0,1)$, ¿Cuál es la distribución de $X$?
...
::: {.pull-right}
<br>
[Chequeamos el resultado]{.bolder}
```{r}
...
[En este caso]{.bold}:
```{r}
z_95 <- qnorm( 1 - 0.1/2 ); print(z_95)
x2 <- 5 + 3*z_95; print(x2)
```
:::

## Hasta la próxima clase. Gracias! {background-color="#FFFFF0" .inverse .center .middle}
