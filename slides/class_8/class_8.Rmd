...
- .bold[Variable degenerada en µ]:  A medida que $n \to \infty$, $\bar{X}$ tiende a transformarse en una constante (variable "degenerada").labs(x=expression(hat(p)), y="Frecuencia")
...
# Create individual plots
plot_n2 <- ggplot(data_n2, aes(sample_sum)) + 
  geom_histogram(fill = "#D15F3E", bins = 11, color = "black", alpha=0.7) +
  ggtitle("2 dados") +
  theme_minimal() +
  labs(x="Suma 2 dados", y="Frecuencia") +
  theme(
    axis.text.y = element_text(size = 22),
    axis.text.x = element_text(size = 22),
    axis.title.y = element_blank(),
...
plot_n5 <- ggplot(data_n5, aes(sample_sum)) + 
  geom_histogram(fill = "#D15F3E", bins = 25, color = "black", alpha=0.7) +
  ggtitle("5 dados") +
  theme_minimal() +
...
plot_n10 <- ggplot(data_n10, aes(sample_sum)) + 
  geom_histogram(fill = "#D15F3E", bins = 50, color = "black", alpha=0.7) +
  ggtitle("10 dados") +
  theme_minimal() +
...
<br>
$\quad \quad \quad \quad$ Por tanto $\frac{1}{\sqrt{n}} \sum_{i=1}^{n}{Z_{i}} \sim \text{Normal}(0, 1)$ 
