...
subtitle: "#8: Teoremas Asintóticos"
...
style_duo_accent(primary_color ="#D15F3E", secondary_color ="#3A7CA5",
...
p <- ggplot() +
  geom_line(data = plot_data, aes(x = x, y = y), color = "#3A7CA5") +
  xlab("n: tamaño muestra (# dados lanzados)") +
  ylab(expression(paste(bar(X)[n], ": promedio muestral"))) + # Updated y-axis label
  ylim(2.5, 4.5) + 
  xlim(1, 6000) + 
  geom_segment(aes(x = 1, y = 3.5, xend = 5000, yend = 3.5), color = "#3A7CA5") + 
  geom_segment(aes(x = 1, y = 3.5 + epsilon, xend = 5000, yend = 3.5 + epsilon), color = "#D15F3E", linetype = "dashed") +
  geom_segment(aes(x = 1, y = 3.5 - epsilon, xend = 5000, yend = 3.5 - epsilon), color = "#D15F3E", linetype = "dashed") +
  annotate("text", x = 5550, y = 3.5, label = "µ = 3.5", size = 5, color = "#3A7CA5") +  # Added text for µ
  annotate("text", x = 3000, y = 3.5 + epsilon + 0.15, label = expression(paste("µ + ", epsilon)), size = 5, color = "#D15F3E") +  # Added text for upper epsilon line
  annotate("text", x = 3000, y = 3.5 - epsilon - 0.15, label = expression(paste("µ - ", epsilon)), size = 5, color = "#D15F3E") +  # Added text for lower epsilon line
  theme_minimal() +
  theme(
...
for (n in 1:5000) {
  ...
  # Update the plot
  if (n %% 5000 == 0) {
    print(p + geom_point(data = plot_data, aes(x = x, y = y), size = 0.5, color = "#3A7CA5", alpha=0.15) +
            geom_line(data = plot_data, aes(x = x, y = y), size = 0.5, color = "#3A7CA5", alpha=0.15)
    )
  }
...
ggplot(data = plot_data, aes(x = sample_size, y = mean_outside_epsilon)) +
  geom_line(color = "#3A7CA5", size = 0.8, alpha=0.7) +
  geom_point(color = "#3A7CA5", size = 1.2) +
  xlab("n: tamaño muestra (# dados lanzados)") +
...
# Plotting
ggplot() +
  geom_line(data = averages_data, aes(x = sample_size, y = 3.5, color = "a"), size = 1) +   
  geom_line(data = averages_data, aes(x = sample_size, y = avg_sample_mean, color = "b"), size = 1, alpha=0.5) +
  scale_color_manual(
    values = c("a" = "#3A7CA5", "b" = "#D15F3E"), 
...
ggplot() +
  ...
  scale_color_manual(
    values = c("Individual Sample Mean" = "#3A7CA5", "Average of Sample Means" = "#D15F3E"), 
...
ggplot(data = sd_data, aes(x = sample_size)) +
  ...
  scale_color_manual(
    values = c("sample_sd" = "#3A7CA5", "theoretical_sd" = "#D15F3E"),
...
.bold[Teorema del Límite Central (parte 1)]: $\bar{X} \sim \text{Normal}\Bigg(\mu, \frac{\sigma}{\sqrt{n}}\Bigg)$
...
plot_n7 <- ggplot(data_n7, aes(sample_mean)) + 
  geom_density(fill = "#D15F3E", alpha = 0.7) +
  xlim(2.5, 4.5) +
  ggtitle("n = 7") +
  theme_minimal() +
  labs(x="Sample mean", y="Density") + # Added y label
  theme(
    axis.text.y = element_text(size = 22),
    axis.text.x = element_text(size = 22),
    axis.title.y = element_text(size = 24),
...
# For plot_n50 and plot_n1000, remove y-axis label by setting axis.title.y = element_blank()
...
- A medida que $n$ aumenta $\bar{X}$ tiende a seguir una .bold[distribución normal], .bold[sin importar la distribución original de las variables aleatorias].

- A medida que $n \to \infty$, $\bar{X}$ tiende a transformarse en una constante (variable "degenerada").


---
## Teorema del Límite Central

.bold[Teorema del Límite Central (parte 2)]: 
<br>
.center[
...
.bold[Teorema del Límite Central (parte 2)]: Si $\bar{X} \sim \text{Normal}\Bigg(\mu, \frac{\sigma}{\sqrt{n}}\Bigg)$
