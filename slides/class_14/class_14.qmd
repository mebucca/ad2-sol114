...
subtitle: "IC con varianza desconocida & Margen de Error"
...
format:
  revealjs:
    ...
    title-slide-attributes:
      data-background-color: "#228B22"
---

# IC con varianza desconocida & Margen de Error {background-color="#4B0082" .inverse .center .middle}
...
::: {.pull-left}
[Promedio de ingreso mujeres]{.bold}
...
[Instrucción:]{.bold} Calcula un intervalo al 99% de confianza y otro al 90%.
...
* [No conocemos la varianza poblacional]{.bold}. Necesitamos estimarla a partir de los datos.
...
[Problema]{.bold}: $\sigma^2_{\text{naive}}$ no es un buen estimador de la $\sigma^2$
...
[Correción]{.bold}: $\mathbb{E}[\big(\frac{n}{n-1} \big) \sigma^2_{\text{naive}}] = \sigma^2 \big(\frac{n-1}{n} \big)\big(\frac{n}{n-1} \big) = \sigma^2$
...
[Instrucción:]{.bold} Calcula un intervalo al 95% de confianza.
...
## Margen de Error {background-color="#4B0082" .inverse .center .middle .section-title}
...
En encuestología es común escuchar el término [margen de error]{.bold}.
...
Por ejemplo, si una encuesta reporta que: <br>

> "58% de los encuestados señalan que votarán "A favor" en el plebiscito de salida" ... "La metodología considera máxima varianza, con un 95% de confianza y 2,8% de margen de error" (sic)
...
- $\sigma^2$ es la varianza poblacional de la variable en cuestión. Usualmente desconocida.
  ...
  - Para variables dicotómicas se asume [varianza máxima]{.bold}. El máximo de $\mathbb{Var}(X) = p(1-p)$ ocurre cuando $p=0.5 \implies p(1-p)=0.25$
...
Por ejemplo, si vamos a estimar una proporción y queremos trabajar con un margen de error de 2 puntos porcentuales (ME = 0.02) al 95% de confianza, entonces necesitamos una muestra de tamaño:
...
[IMPORTANTE:]{.bold} El margen de error es específico a una variable. NO existe tal cosa como el [margen de error de la encuesta]{.bold}.

## {background-color="#4B0082" .inverse .middle}
...
## Hasta la próxima clase. Gracias! {background-color="#4B0082" .inverse .center .middle}
