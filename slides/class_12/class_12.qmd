...
format:
  revealjs:
    ...
    title-slide-attributes:
      data-background-color: "#1F3C88"
---

## Estimación de Intervalos {background-color="#DAA996" .inverse .center .middle .section-title}
...
- No nos tomamos este número tan en serio
:::
...
## Error en estimación {background-color="#DAA996" .inverse .center .middle .section-title}
...
dividiendo por el error estándar (SE) de $\bar{X}$: $\quad \quad \mathbb{P}\bigg(\frac{-e }{\sigma/\sqrt{n}} < Z_{\bar{X}}  < \frac{e}{\sigma/\sqrt{n}}\bigg) =$
...
## Error en estimación, ejemplo
  ...
  - Tomamos una muestra aleatoria de tamaño 200 y estimamos la media muestral $\bar{X}_{200}$.
  

. . .

[¿Cual es la probabilidad de que nuestro estimado difiera como máximo en 0.25 puntos respecto de la media poblacional?]{.bold} (en cualquier dirección)
...
Dividimos por el error estándar (SE) de $\bar{X}$, donde  $SE_{\bar{X}} = \sigma/\sqrt{n} =  3.464/\sqrt{200} \approx 0.245$
...
## Intervalos de confianza {background-color="#DAA996" .inverse .center .middle .section-title}
...
Un [intervalo de confianza]{.bold} consiste en el proceso inverso:

> Buscar un intervalo de valores tal que el [parámetro poblacional se encuentre dentro del intervalo]{.bold} con una cierta probabilidad arbitraria (nivel de confianza). 

<br>

. . .

Definamos un [Nivel de Confianza]{.bold} = $1 - \alpha$ 
...
[¿Cómo determinamos los valores de a y b?]{.bold}
...
## Hasta la próxima clase. Gracias! {background-color="#DAA996" .inverse .center .middle}
