...
## Ley de los Grandes Números {background-color="#F4C430" .inverse .center .middle .section-title}
...
- La media aritmética en [esta muestra]{.bold} es  $\bar{X}_{n} = \frac{1}{n}\sum_{i=1}^{n}X_i$

- $\bar{X}_{n}$ es [aleatoria]{.bold} porque las $X_{i}$'s son aleatorias.

<br>

. . .

[Ley de los Grandes Números]{.bold} establece que, a medida que el tamaño de una muestra aleatoria crece, la media aritmética de la muestra [converge en probabilidad]{.bold} al promedio teórico/poblacional: $\bar{X} \overset{p}{\to} \mathbb{E}(X)$.

::: {.content-box-primary}
[Matemáticamente,]{.bolder}
$$\color{white}{\lim_{n \to \infty}\mathbb{P}\left( \left| \bar{X} - \mu \right| \geq \epsilon \right) = 0}$$
:::

## Ley de los Grandes Números

[Convergencia en Probabilidad]{.bold}
...
## Valor Esperado y Varianza {background-color="#F4C430" .inverse .center .middle .section-title}
# de la
# Media Muestral
...
- Las $X_{i}$'s son [iid]{.bold}
    
- En cada muestra calculamos la [media muestral]{.bold},  $\bar{X}_{n}$

- $\bar{X}_{n}$ es [aleatoria]{.bold} porque las $X_{i}$'s son aleatorias.

<br>

[¿Cuál es el promedio de dichas medias muestrales?]{.bold}
:::
...
[¿Cuanto varían dichas medias muestrales?]{.bold}
:::
...
A medida que aumenta $n$ la desviación estándar de $\bar{X}_{n}$'s [disminuye a una tasa decreciente]{.bold} específica.
...
Típicamente contamos con [sólo una muestra]{.bold}, con un $n$ fijo. 

[¿"Cuán bien" nuestro estimado muestral]{.bold} $\bar{X}_{n}$ [captura el parámetro verdadero E(X)?]{.bold} 
