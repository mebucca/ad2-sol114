...
subtitle: "Intervalos de Confianza via Método de Bootstrap"
...
format:
  revealjs:
    ...
    title-slide-attributes:
      data-background-color: "#744B22"
---

# Intervalos de Confianza via Método de Bootstrap {background-color="#3B4082" .inverse .center .middle}
...
- Sin embargo, incluso para estimadores simples puede ser difícil (o imposible) determinar la distribución muestral (ej. mediana).
...
2. Aplica el estimador $\theta(X_{b})$ a la muestra $(X_{b})$ para obtener el estimado $\hat{\theta}_{b}$
...
[Calculemos un IC al 99% para la `media` muestral del ingreso de las mujeres]{.bold}
...
dismuestral_bs_plot <- ggplot(data=distmuestral_boot) +
  geom_histogram(aes(x=value, y = after_stat(density)), fill = secondary_color, bins = 100, color=secondary_color, alpha=0.4) +
...
[Calculemos un IC al 99% para la `mediana` muestral del ingreso de las mujeres]{.bold}
...
## Hasta la próxima clase. Gracias! {background-color="#3B4082" .inverse .center .middle}
