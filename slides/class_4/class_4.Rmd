...
ggplot(data = PMF_df, aes(x = factor(x), y = PMF)) +
  geom_bar(stat = "identity", color = "black", fill = "#ffec73", width = 1, alpha=0.8) +
  labs(x = "Outcome",
       y = "P(X = x)") +
  scale_x_discrete(labels = c("Sello (0)", "Cara (1)")) +
  scale_color_manual(values = "#ffec73") +
...
.content-box-primary[
.bolder[Pregunta:]
¿Cuál es la probabilidad de conseguir $x$ "Caras" con $n$ tiros?
]

--

* La probabilidad de obtener una secuencia particular con $x$ "Caras" a partir de $n$ lanzamientos es $p^{x}(1-p)^{n-x}$ 

* Existen ${n \choose x} = \frac{n!}{x! (n-x)!}$ secuencias de este tipo...

--
Por tanto,

$$\mathbb{P}(Y=x) = f(x) = \frac{n!}{x! (n-x)!} \times p^{x} (1-p)^{n-x}$$
--
En otras palabras, $X$ distribuye binomial con .bold[parámetros] $n$ y $p$: $X \sim \text{Binomial}(n,p)$
...
$\quad \quad \quad \mathbb{P}(Y=3) = \binom{10}{3} \times (0.6)^3 \times (0.4)^{10-3}$

<br>
--

$\quad \quad \quad \mathbb{P}(Y=3) = \frac{10!}{3! \times 7!} \times (0.6)^3 \times (0.4)^7$

<br>
--

$\quad \quad \quad \mathbb{P}(Y=3) = \frac{10 \times 9 \times \dots 1}{(3 \times 2 \times1) \times (7 \times 6 \times \dots 1 )} \times (0.6)^3 \times (0.4)^7$

<br>
--

$\quad \quad \quad \mathbb{P}(Y=3) = 120 \times 0.216 \times 0.0028$

<br>
--

$\quad \quad \quad \mathbb{P}(Y=3) \approx 0.072$
...
ggplot(data = PMF_df, aes(x = factor(x), y = PMF)) +
  geom_bar(stat = "identity", color = "black", fill = "#ffec73", width = 1, alpha=0.8) +
  geom_bar(data = overlay_df, aes(x = factor(x), y = PMF), stat = "identity", color = "black", fill = "#ff7377", width = 1, alpha=0.6) +
  labs(x = "Number of Successes",
       y = "P(X = x)") +
  scale_x_discrete(labels = as.character(0:10)) +
  scale_color_manual(values = "black") +
  scale_fill_manual(values = c("#ffec73", "#ff7377")) +
  theme_minimal() +
  theme(panel.background = element_rect(color = "black", fill = NA, size = 1))  # This line adds the inner border
...
ggplot() +
  ...
  geom_segment(data = CDF_df, aes(x = start_x, y = CDF, xend = x, yend = CDF), color = "#ffec73", size = 1.2) +
  # Vertical segments for CDF
  geom_segment(data = CDF_df, aes(x = x-1, y = start_CDF, xend = x-1, yend = CDF), color = "#ffec73", size = 1.2) +
  
  # Horizontal segments for the overlay
  geom_segment(data = overlay_df, aes(x = start_x, y = CDF, xend = x, yend = CDF), color = "#ff7377", size = 1.2, alpha = 0.6) +
  # Vertical segments for the overlay
  geom_segment(data = overlay_df, aes(x = x-1, y = start_CDF, xend = x-1, yend = CDF), color = "#ff7377", size = 1.2, alpha = 0.6) +
...
In this code, `geom_area` is used to fill the area under the curve with the color `#ffec73` and an alpha value of `0.3` for transparency. The rest of the code remains the same as the one you provided.
