...
format:
  revealjs:
    ...
    title-slide-attributes:
      data-background-color: "#00634E"
---

# Asociación entre variables continuas {background-color="#003E30" .inverse .center .middle}
...
## Covarianza {background-color="#003E30" .inverse .center .middle .section-title}
...
## Correlación de Pearson {background-color="#003E30" .inverse .center .middle .section-title}
...
- [Solución:]{.bold} Estandarizar. En vez de calcular $\mathbb{Cov}(X, Y)$ calculamos $\mathbb{Cov}(Z_X, Z_Y)$. Formalmente,
...
El [Coeficiente de Correlación de Pearson]{.bold} se define como:
...
- $\rho = 0$ sugiere que no hay asociación lineal entre las variables. Los valores que se acercan a cero indican una débil o nula correlación lineal.
...
- [Corr(esc,log-ingreso) | Hombre]{.bold} = `r casen2017 %>% filter(genero=="Hombre") %>% with(cor(esc,log(ingreso), use = "complete.obs"))`

- [Corr(esc,log-ingreso) | Mujer]{.bold} = `r casen2017 %>% filter(genero=="Mujer") %>% with(cor(esc,log(ingreso), use = "complete.obs"))`

## Correlación Lineal de Pearson

[Importante]{.bold}: La covarianza y correlación miden el grado de [asociación lineal]{.bold}.
...
::: {.pull-right}
[Asociación no lineal perfecta ]{.bold}
...
## The End. Gracias! {background-color="#003E30" .inverse .center .middle}
