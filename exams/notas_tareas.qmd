...
media <- mean(notas_tareas$tarea_promedio)
mediana <- median(notas_tareas$tarea_promedio)
sd <- sd(notas_tareas$tarea_promedio)
min_val <- min(notas_tareas$tarea_promedio)
max_val <- max(notas_tareas$tarea_promedio)

# Set limits for y-axis so the annotations have enough space
limits_y <- c(0, max(density(notas_tareas$tarea_promedio)$y)*1.5)

# Define ggplot
p <- ggplot(notas_tareas, aes(x = tarea_promedio)) +
...
fit <- Mclust(notas_tareas$tarea_promedio, G = 4, adjustment = list(diagCov = TRUE))

# Get the cluster assignments for each data point
cluster_assignments <- fit$classification

# Calculate the means for each cluster
cluster_means <- tapply(notas_tareas$tarea_promedio, cluster_assignments, mean)

# Create a ggplot without a legend, add horizontal lines, annotate cluster means, and add vertical lines
p <- ggplot(notas_tareas, aes(x = tarea_promedio, fill = factor(cluster_assignments))) +
...
# Data preparation
data_long <- notas_tareas %>%  select(Rut,starts_with("tarea")) %>%
  pivot_longer(cols = starts_with("tarea"), names_to = "Variable", values_to = "Value")
