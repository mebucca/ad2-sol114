...
Dado que el error estándar es $SD(\hat{\beta}) = E[ (\hat{\beta} - E(\hat{\beta}))^2]$ entonces cuando un estimador es insesgado el error estándar mide cuánto se desvían las estimaciones individuales respecto de $E(\hat{\beta})$, que es igual a $\beta$.
Sin embargo, si el estimador está sesgado, el error estándar no nos dirá nada sobre cuánto se desvían las estimaciones del verdadero valor poblacional $\beta$, sino $E(\hat{\beta}) \neq \beta$.
