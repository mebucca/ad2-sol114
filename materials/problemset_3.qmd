...
$$\bar{X}_{\text{final}} = \frac{4.5 + 3.8 + 5.2}{3} = \frac{13.5}{3} = 4.5$$

La media muestral del puntaje final para estos 100 estudiantes es 4.5.

\textbf{Intervalo de Confianza del 95\% para la Media del Puntaje Final}

Dado que el puntaje final es el promedio de tres pruebas independientes, su varianza se obtiene aplicando la propiedad de varianza de una combinación lineal. Si $\bar{X}_{\text{final}} = \frac{X_1 + X_2 + X_3}{3}$, entonces:

$$\text{Var}(\bar{X}_{\text{final}}) = \frac{1}{3^2}\left(\text{Var}(X_1) + \text{Var}(X_2) + \text{Var}(X_3)\right) = \frac{s^2_1 + s^2_2 + s^2_3}{9}$$

Las varianzas muestrales ($s^2 = \frac{\sum (x_i - \bar{x})^2}{n-1}$) para cada prueba son:

$$s^2_{\text{entradas}} = \frac{375.6}{99} \approx 3.794$$

$$s^2_{\text{plato principal}} = \frac{178.4}{99} \approx 1.802$$

$$s^2_{\text{postres}} = \frac{62.7}{99} \approx 0.633$$

La varianza del puntaje final es:

$$s^2_{\text{final}} = \frac{3.794 + 1.802 + 0.633}{9} = \frac{6.229}{9} \approx 0.692$$

La desviación estándar del puntaje final:

$$s_{\text{final}} = \sqrt{0.692} \approx 0.832$$

El error estándar de la media del puntaje final:

$$s_{\bar{X}_{\text{final}}} = \frac{s_{\text{final}}}{\sqrt{n}} = \frac{0.832}{\sqrt{100}} = \frac{0.832}{10} = 0.0832$$

Usando la distribución $Z$ para un intervalo de confianza del 95\% ($Z_{0.025} = 1.96$):

$$\text{IC} = \bar{X}_{\text{final}} \pm \left(Z \cdot s_{\bar{X}_{\text{final}}}\right)$$

$$\text{IC} = 4.5 \pm (1.96 \times 0.0832)$$

$$\text{IC} = 4.5 \pm 0.163$$

$$\boxed{\text{IC} = [4.337,\ 4.663]}$$

**Interpretación del Intervalo de Confianza**

Estamos 95\% seguros de que la media verdadera del puntaje final para todos los estudiantes que podrían participar en esta competencia se encuentra entre 4.34 y 4.66. Nótese que este intervalo es más estrecho que el que se obtendría considerando la varianza de cualquier prueba individual, lo cual refleja correctamente que promediar tres pruebas independientes reduce la variabilidad del puntaje final.
